      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <p className="text-blue-600 font-medium mb-2"><AutoText>OUR APPROACH</AutoText></p>
            <h2 className="text-3xl md:text-4xl font-bold text-slate-900"><AutoText>Science-Led Innovation</AutoText></h2>
          </div>
              ...
              return (
                <Link key={idx} href={feature.href} className="group">
                  <div className="bg-slate-50 rounded-2xl p-8 hover:shadow-xl transition-all h-full">
                    ...
                    <h3 className="text-xl font-bold text-slate-900 mb-4 group-hover:text-blue-600 transition-colors"><AutoText>{feature.title}</AutoText></h3>
                    <p className="text-slate-600 mb-6 leading-relaxed"><AutoText>{feature.description}</AutoText></p>
                    <div className="flex items-center text-blue-600 font-medium">
                      <span><AutoText>Learn more</AutoText></span>
      ...
      <section className="py-20 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <p className="text-blue-600 font-medium mb-2"><AutoText>THERAPEUTIC FOCUS</AutoText></p>
            <h2 className="text-3xl md:text-4xl font-bold text-slate-900"><AutoText>Areas of Impact</AutoText></h2>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {impactAreas.map((area: any, idx: number) => (
              <div key={idx} className="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
                ...
                <h3 className="font-semibold text-slate-900 mb-2"><AutoText>{area.title}</AutoText></h3>
                <p className="text-sm text-slate-600"><AutoText>{area.description}</AutoText></p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-8">
            <Link href={ctaCards[0]?.href || '/what-science-can-do/stories-of-impact'} className="group bg-slate-900 rounded-2xl p-8 text-white hover:bg-slate-800 transition-colors">
              ...
              <h3 className="text-2xl font-bold mb-3"><AutoText>{ctaCards[0]?.title || 'Stories of Impact'}</AutoText></h3>
              <p className="text-slate-300 mb-6"><AutoText>{ctaCards[0]?.description}</AutoText></p>
              <div className="flex items-center text-blue-400 font-medium">
                <span><AutoText>{ctaCards[0]?.linkLabel || 'Explore stories'}</AutoText></span>
            ...
            <Link href={ctaCards[1]?.href || '/what-science-can-do/publications'} className="group bg-blue-600 rounded-2xl p-8 text-white hover:bg-blue-700 transition-colors">
              ...
              <h3 className="text-2xl font-bold mb-3"><AutoText>{ctaCards[1]?.title || 'Publications'}</AutoText></h3>
              <p className="text-blue-100 mb-6"><AutoText>{ctaCards[1]?.description}</AutoText></p>
              <div className="flex items-center text-white font-medium">
                <span><AutoText>{ctaCards[1]?.linkLabel || 'View publications'}</AutoText></span>
                <ChevronRight className="w-5 h-5 ml-1 group-hover:translate-x-1 transition-transform" />
              </div>
            </Link>
          </div>
      ...
      <section className="py-20 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-8 md:p-12 text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4"><AutoText>{partnerCta.title}</AutoText></h2>
            <p className="text-slate-300 mb-8 max-w-2xl mx-auto"><AutoText>{partnerCta.description}</AutoText></p>
            <div className="flex flex-wrap justify-center gap-4">
              <Link href={partnerCta.primaryLink?.href || '/contact'} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-medium transition-colors">
                <AutoText>{partnerCta.primaryLink?.label || 'Contact Our Team'}</AutoText>
              </Link>
              <Link href={partnerCta.secondaryLink?.href || '/about'} className="border border-white/30 hover:border-white text-white px-6 py-3 rounded-lg font-medium transition-colors">
                <AutoText>{partnerCta.secondaryLink?.label || 'Learn About Us'}</AutoText>
              </Link>
            </div>
          </div>
        </div>
