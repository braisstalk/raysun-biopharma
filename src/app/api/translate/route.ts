...
function fixBrandNames(text: string, lang: string): string {
  if (lang === 'zh') {
    // 匹配任何以"瑞"或"雷"开头，后面跟着1-2个任意字符，然后是"生物制药"或"制药"的模式
    return text
      .replace(/瑞[\u4e00-\u9fa5]{1,2}生物制药/g, '雷神生物制药')
      .replace(/瑞[\u4e00-\u9fa5]{1,2}制药/g, '雷神制药')
      .replace(/雷[\u4e00-\u9fa5&&[^神]]{1,2}生物制药/g, '雷神生物制药')
      .replace(/雷[\u4e00-\u9fa5&&[^神]]{1,2}制药/g, '雷神制药')
